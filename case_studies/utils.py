...
def generate_marketing_data(
    ...
    场景: 外卖/电商平台优惠券分配
    - 特征: 用户画像 (年龄、消费习惯、会员状态等)
    - 处理: 是否发放优惠券
    - 结果: 是否转化购买
    - 混淆因素: 高价值用户更容易被选中发券，也更容易转化
    ...
    Returns:
    --------
    (df, true_uplift, gmv_lift): 数据、真实uplift、GMV增益
    """
    if seed is not None:
        np.random.seed(seed)
    ...
    order_frequency = np.random.poisson(5, n_samples) + 1
    days_since_last_order = np.random.exponential(15, n_samples)
    is_member = np.random.binomial(1, 0.3, n_samples)
    app_sessions = np.random.poisson(10, n_samples)
    city_tier = np.random.choice([1, 2, 3], n_samples, p=[0.3, 0.4, 0.3])

    # 标准化
    age_norm = (age - 35) / 12
    aov_norm = (avg_order_value - 60) / 30
    freq_norm = (order_frequency - 5) / 3
    days_norm = (days_since_last_order - 15) / 10
    ...
    baseline_conversion = np.clip(baseline_conversion, 0.05, 0.5)

    # === 处理效应 (发券的异质性效果) ===
    # 低频用户、年轻用户更敏感
    true_uplift = (
        0.10 +
        -0.05 * freq_norm +  # 低频用户 uplift 高
        0.03 * (age < 30) +  # 年轻用户更敏感
        -0.02 * is_member +  # 会员本身就会买，uplift 低
        0.04 * (days_since_last_order > 20)  # 流失用户 uplift 高
    )
    true_uplift = np.clip(true_uplift, -0.05, 0.25)

    # === 转化结果 ===
    conversion_prob = baseline_conversion + true_uplift * T
    conversion_prob = np.clip(conversion_prob, 0, 1)
    conversion = np.random.binomial(1, conversion_prob)

    # === GMV ===
    gmv = conversion * avg_order_value
    gmv_lift = (gmv[T == 1].mean() - gmv[T == 0].mean()) / gmv[T == 0].mean()
    ...
    return df, true_uplift, gmv_lift
...
def compute_roi(
    ...
    Parameters:
    -----------
    df: 包含 T, conversion 列的数据
    scores: 用于排序的分数 (越高越优先发券)
    target_fraction: 发券比例
    revenue_per_conversion: 每次转化收入
    cost_per_treatment: 每次发券成本

    Returns:
    --------
    Dict with roi, revenue, cost, conversions
    """
    n_target = int(len(df) * target_fraction)

    # 按分数排序，选择 top N
    top_indices = np.argsort(scores)[-n_target:]

    # 在这些用户中，假设我们发券
    # 估计转化率 (使用实际数据中的处理组转化率)
    df_top = df.iloc[top_indices]

    # 使用处理组的实际转化率作为预期转化率
    if (df_top['T'] == 1).sum() > 0:
        expected_conversion_rate = df_top[df_top['T'] == 1]['conversion'].mean()
    else:
        expected_conversion_rate = df['conversion'].mean()

    expected_conversions = n_target * expected_conversion_rate
