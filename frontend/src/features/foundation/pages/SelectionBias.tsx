...
import { useSelectionBias } from '../hooks'
...
export function Component() {
  ...
  const { mutate, data, isPending, error } = useSelectionBias()
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-foreground">选择偏差</h1>
        <p className="text-muted-foreground mt-1">
          Selection Bias - 当样本选择与结果相关时
        </p>
      </div>
      ...
      <div className="rounded-lg border bg-card p-6">
        ...
        <div className="grid gap-6 md:grid-cols-3">
          ...
          <Slider
            ...
            max={3}
            step={0.1}
            value={[params.selection_strength]}
            onValueChange={([v]) => setParams((p) => ({ ...p, selection_strength: v }))}
          />
          <Slider
            label="真实处理效应"
            min={-5}
